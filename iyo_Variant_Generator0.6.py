# iyo_Variant_Generator0.6.py (Version 17.3 - Input Trimming)
...
# --- CORE LOGIC (No changes in this version) ---
...
def get_default_profile():
    default_percentages = [50, 60, 70, 80, 90, 110, 120, 130, 140, 150, 200]; default_durations = [15, 30, 45, 60, 90, 120]
    profile = { 
        "folder_path": DEFAULT_KOVAAKS_PATH, "percentages_size": default_percentages.copy(), "percentages_speed": default_percentages.copy(), 
        "percentages_timescale": default_percentages.copy(), "durations": default_durations.copy(), "checkboxes": {},
    ...
    try:
        with open(SETTINGS_FILE, 'r') as f:
            ...
            if "profiles" in settings and "last_active_profile" in settings: return settings
                ...
                return {"last_active_profile": "Default", "profiles": {"Default": settings}}
...
class VariantGeneratorApp:
    def __init__(self, root):
        ...
        self.header_font = font.Font(family=default_font.cget("family"), size=default_font.cget("size")+2, weight="bold")
        self._create_widgets(); self._load_profile(self.active_profile_name); self._populate_scenario_list()
    ...
    def _create_widgets(self):
        ...
        self.folder_path_var = tk.StringVar(); ttk.Label(frame1, text="Folder Path:").grid(row=0, column=0, sticky="w", padx=5); ttk.Entry(frame1, textvariable=self.folder_path_var, width=80).grid(row=0, column=1, sticky="ew", padx=5); ttk.Button(frame1, text="Browse...", command=self._on_browse).grid(row=0, column=2, padx=5)
        self.scenario_name_var = tk.StringVar(); self.scenario_name_var.trace_add("write", self._schedule_load_from_entry)
        ttk.Label(frame1, text="Scenario Name:").grid(row=1, column=0, sticky="w", padx=5); ttk.Entry(frame1, textvariable=self.scenario_name_var, width=80).grid(row=1, column=1, sticky="ew", padx=5)
        list_frame = ttk.Frame(frame1); list_frame.grid(row=2, column=1, sticky="ew", padx=5, pady=(5,0))
        self.scenario_listbox = tk.Listbox(list_frame, height=6, exportselection=False); self.scenario_listbox.pack(side="left", fill="both", expand=True)
        scrollbar = ttk.Scrollbar(list_frame, orient="vertical", command=self.scenario_listbox.yview); scrollbar.pack(side="right", fill="y")
        self.scenario_listbox.config(yscrollcommand=scrollbar.set); self.scenario_listbox.bind("<<ListboxSelect>>", self._on_listbox_select)
        ...
        ttk.Label(frame_profiles, text="Active Profile:").pack(side="left", padx=(0, 5)); self.profile_combobox = ttk.Combobox(frame_profiles, state="readonly", width=30); self.profile_combobox.pack(side="left", padx=5); self.profile_combobox.bind("<<ComboboxSelected>>", self._on_profile_select)
        ttk.Button(frame_profiles, text="Save As...", command=self._on_save_profile_as).pack(side="left", padx=5); ttk.Button(frame_profiles, text="Rename", command=self._on_rename_profile).pack(side="left", padx=5); ttk.Button(frame_profiles, text="Delete", command=self._on_delete_profile).pack(side="left", padx=5)
        
        frame2 = ttk.LabelFrame(main_frame, text="2. Detected Base Stats", padding="10"); frame2.grid(row=2, column=0, sticky="ew", pady=5)
        frame2.columnconfigure(1, weight=1); frame2.columnconfigure(3, weight=1)
        self.stat_vars = { 
            "Scenario Name:": tk.StringVar(value="Scenario Name: N/A"), "Target(s):": tk.StringVar(value="N/A"), "Target Radius:": tk.StringVar(value="N/A"), 
            "Target Max Speed:": tk.StringVar(value="N/A"), "Timescale:": tk.StringVar(value="N/A"), "Duration:": tk.StringVar(value="N/A")
        }
        scen_name_var = self.stat_vars["Scenario Name:"]
        scen_name_label = ttk.Label(frame2, textvariable=scen_name_var, font=self.header_font, anchor="center")
        scen_name_label.grid(row=0, column=0, columnspan=4, sticky="ew", pady=(0, 5))
        other_stats = {k: v for k, v in self.stat_vars.items() if k != "Scenario Name:"}
        items = list(other_stats.items()); num_rows = (len(items) + 1) // 2 
        for i, (text, var) in enumerate(items):
            row = (i % num_rows) + 1; col = (i // num_rows) * 2
            ttk.Label(frame2, text=text, width=15).grid(row=row, column=col, sticky="w", padx=5)
            ttk.Label(frame2, textvariable=var).grid(row=row, column=col + 1, sticky="w")
    ...
    def _on_browse(self):
        ...
        if folder: self.folder_path_var.set(folder); self._populate_scenario_list(); self._on_settings_change()
    ...
    def _create_variant_column(self, parent, vtype_key, values, suffix, display_name):
        ...
        header_label = ttk.Label(frame, text=f"{display_name} Variants"); header_entry = ttk.Entry(frame, textvariable=header_var, width=12)
        ...
        for i, val in enumerate(values):
            row_frame = ttk.Frame(frame); row_frame.pack(anchor="w"); key = f"{vtype_key}_{i}"
    ...
    def _load_profile(self, profile_name):
        ...
        self.folder_path_var.set(profile_data["folder_path"]); self._build_variant_columns()
        for key, value in profile_data["checkboxes"].items():
            if key in self.checkbox_vars: self.checkbox_vars[key].set(value)
        self._update_profile_dropdown(); self.is_edit_mode = False
    ...
    def _on_profile_select(self, event=None):
        ...
        if new_profile_name != self.active_profile_name: self._on_settings_change(); self._load_profile(new_profile_name)
    ...
    def _on_load(self):
        # --- MODIFIED: Strip whitespace from user input ---
        user_typed_name = self.scenario_name_var.get().strip()
        ...
        if self.loaded_scenario_data:
            ...
            self.stat_vars["Scenario Name:"].set(f"Scenario Name: {user_typed_name}")
            ...
            messagebox.showerror("Error", f"Found '{user_typed_name}.sce' but could not read or parse it. The file may be corrupted."); self.generate_button.config(state="disabled")
    ...
    def _on_closing(self):
        self._on_settings_change(); save_settings(self.settings); self.root.destroy()
