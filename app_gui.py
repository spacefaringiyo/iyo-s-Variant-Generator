...
class VariantGeneratorApp:
    ...
    def _configure_styles(self):
        ...
        style.configure("Accent.TButton", foreground=MATRIX_GREEN)
    ...
    def _select_background(self):
        def task(): return filedialog.askopenfilename(parent=self.root, filetypes=[("Images", "*.png;*.jpg;*.jpeg;*.bmp")])
    ...
    def _create_widgets(self):
        ...
        self.label_folder_path = ttk.Label(self.frame1); self.label_folder_path.grid(row=0, column=0, sticky="w", padx=5, pady=2)
        ttk.Entry(self.frame1, textvariable=self.folder_path_var, width=80).grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        btn_box = ttk.Frame(self.frame1); btn_box.grid(row=0, column=2, padx=5, pady=2)
        self.button_browse = ttk.Button(btn_box, command=self._on_browse); self.button_browse.pack(side="left")
        ttk.Button(btn_box, text="Open Folder", width=12, command=self._open_folder).pack(side="left", padx=2)
        self.scenario_name_var = tk.StringVar(); self.scenario_name_var.trace_add("write", self._schedule_load_from_entry)
        self.label_scenario_name = ttk.Label(self.frame1); self.label_scenario_name.grid(row=1, column=0, sticky="w", padx=5, pady=2)
        ttk.Entry(self.frame1, textvariable=self.scenario_name_var, width=80).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        self.batch_mode_var = tk.BooleanVar(value=False)
        self.batch_chk = tk.Checkbutton(self.frame1, text="Batch Mode", variable=self.batch_mode_var, command=self._toggle_batch_mode, bg=DARK_BG, fg=LIGHT_TEXT, selectcolor=ENTRY_BG, activebackground=DARK_BG, activeforeground=ACCENT_COLOR, borderwidth=0, highlightthickness=0, font=("Consolas", 10, "bold"))
        self._style_checkbox_dynamic(self.batch_chk, self.batch_mode_var)
        self.batch_chk.grid(row=1, column=2, padx=5)

        list_frame = ttk.Frame(self.frame1); list_frame.grid(row=2, column=1, sticky="ew", padx=5, pady=(5,0))
        ...
        # --- Frame 2 ---
        self.frame2 = ttk.LabelFrame(self.ui_window, padding="10", text="📊 2. Base Stats")
        self.frame2.grid(row=3, column=0, sticky="ew", pady=5, padx=15)
        ...
        scen_name_label = ttk.Label(self.single_mode_frame, textvariable=scen_name_var, font=self.header_font, anchor="center", foreground=ACCENT_COLOR)
        ...
        def add_stat_row(r, label_key, val_key, col_offset=0):
            ...
            return lbl
        self.label_target = add_stat_row(1, "stats_targets", "Target(s):")
        ...
        self.bot_selection_frame = ttk.LabelFrame(self.single_mode_frame, padding="10")
        self.bot_selection_frame.grid(row=6, column=0, columnspan=4, sticky="ew", padx=5, pady=(15, 5))
        ...
        # --- Frame 3 ---
        self.frame3 = ttk.LabelFrame(self.ui_window, padding="10", text="🚀 3. Create Variants")
        self.frame3.grid(row=4, column=0, sticky="ew", pady=5, padx=15)
        generate_frame = ttk.Frame(self.ui_window); generate_frame.grid(row=5, column=0, sticky="ew", pady=10, padx=15)
        self.generate_button = ttk.Button(generate_frame, command=self._on_generate, state="disabled")
        self.generate_button.pack(side="left", pady=5)
        self.progress_bar = ttk.Progressbar(generate_frame, orient='horizontal', length=500, mode='determinate')
        self.progress_bar.pack(side="left", fill="x", expand=True, pady=5, padx=(20,10))
    ...
    def _build_variant_columns(self):
        ...
        for vtype_key, config in self.variant_configs.items():
            ...
            widgets['frame'].grid(row=1, column=col_index, padx=10, sticky="ns"); config['widgets'] = widgets; col_index += 1
    ...
    def _on_load(self):
        ...
        for widget in self.bot_selection_frame.winfo_children(): widget.destroy()
        self.bot_selection_vars = {}
        ...
        if self.loaded_scenario_data:
            ...
            if target_names:
                ...
                btn_frame = ttk.Frame(self.bot_selection_frame); btn_frame.pack(anchor='w', pady=(0,5))
                ...
                # Removed deselect all
                checkbox_container = ttk.Frame(self.bot_selection_frame); checkbox_container.pack(fill='x')
